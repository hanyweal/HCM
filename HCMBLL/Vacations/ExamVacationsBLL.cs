...
namespace HCMBLL
{
    public class ExamVacationsBLL : BaseVacationsBLL
    {
        public override VacationsTypesEnum VacationType
        {
            get
            {
                return VacationsTypesEnum.Exam;
            }
        }
        ...
        {
            float vacationConsumedDays = 0;
            List<BaseVacationsBLL> ExamVacations = this.GetByEmployeeCodeVacationTypeNotCanceled(EmployeeCodeID, ExceptVacationID).ToList()  ;
            if (ExamVacations.Count > 0)
            {
                foreach (var vacation in ExamVacations)
                {
                    ExamVacationsBLL _vacation = this.GetByVacationID(vacation.VacationID) as ExamVacationsBLL;
                    vacationConsumedDays += (_vacation.VacationEndDate.Subtract(_vacation.VacationStartDate).Days + 1);
                }
