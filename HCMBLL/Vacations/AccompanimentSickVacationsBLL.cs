...
namespace HCMBLL
{
    public class AccompanimentSickVacationsBLL : BaseVacationsBLL
    {
        public override VacationsTypesEnum VacationType
        {
            get
            {
                return VacationsTypesEnum.AccompanimentSick;
            }
        }
        ...
        public int AccompanimentSickLeaveDaysLimit
        {
            get
            {
        ...
        internal override Vacations DALInstance
        {
            get
            {
                ...
                Vacation.SickVacationTypeID = (int)this.VacationType == (Int16)VacationsTypesEnum.AccompanimentSick ? (int)this.SickVacationType : (int?)null;
                return Vacation;
        ...
        public int GetDaysWithTwoDates(int EmployeeCodeID, DateTime StartDate, DateTime EndDate)
        {
            ...
            int DaysCounter = 0;
            List<BaseVacationsBLL> AccompanimentSickVacations = this.GetByEmployeeCodeVacationType(EmployeeCodeID);
            ...
            for (DateTime i = StartDate; i <= EndDate;)
            {
                ...
                foreach (BaseVacationsBLL item in AccompanimentSickVacations)
                {
        ...
        public List<VacationBalanceTable> GetBalanceTable(int EmployeeCodeID, DateTime? VacationStartDat, int VacationID)
        {
            ...
            List<BaseVacationsBLL> AccompanimentSickLeaves = GetByEmployeeCodeVacationTypeNotCanceled(EmployeeCodeID).OrderBy(e => e.VacationStartDate).ToList();
            int index = 0;
            ...
            {
                BaseVacationsBLL FirstVacation = new AccompanimentSickVacationsBLL().GetByEmployeeCodeVacationTypeNotCanceled(EmployeeCodeID).FirstOrDefault();
            ...
            if (VacationID > 0)
                AccompanimentSickLeaves = AccompanimentSickLeaves.Where(s => s.VacationID != VacationID).ToList();

            foreach (AccompanimentSickVacationsBLL AccompanimentSickLeave in AccompanimentSickLeaves)
            {
                if (index == 0)
                {
                    // User try to get new vacation before the first vacation in vacation table.
                    if (VacationStartDat < AccompanimentSickLeave.VacationStartDate)
                    {
                        VacationBalanceTable.Add(new VacationBalanceTable()
                        {
                            EmployeeCodeID = EmployeeCodeID,
                            StartDate = VacationStartDat.Value,
                            EndDate = VacationStartDat.Value.AddDays(this.AccompanimentSickLeaveDaysLimit),
                            ConsumedBalance = 0,
                            RemainingBalance = this.VacationBalance
                        });
                        PreviousRowEndDate = VacationStartDat.Value.AddDays(this.AccompanimentSickLeaveDaysLimit).AddDays(1);

                        // check if vacation lies between balanceTable rows, if exists update comsumed else added new row
                        VacationBalanceRow = VacationBalanceTable.FirstOrDefault(v => v.StartDate <= AccompanimentSickLeave.VacationStartDate && v.EndDate >= AccompanimentSickLeave.VacationEndDate);
                        if (VacationBalanceRow != null && VacationBalanceRow.EmployeeCodeID > 0)
                        {
                            VacationBalanceRow.ConsumedBalance += AccompanimentSickLeave.VacationPeriod;
                        ...
                        else
                        {
                            ...
                            {
                                EmployeeCodeID = EmployeeCodeID,
                                StartDate = AccompanimentSickLeave.VacationStartDate,
                                EndDate = AccompanimentSickLeave.VacationStartDate.AddDays(this.AccompanimentSickLeaveDaysLimit),
                                ConsumedBalance = AccompanimentSickLeave.VacationPeriod,
                                RemainingBalance = this.VacationBalance - AccompanimentSickLeave.VacationPeriod
                            });
                            PreviousRowEndDate = AccompanimentSickLeave.VacationStartDate.AddDays(this.AccompanimentSickLeaveDaysLimit).AddDays(1);
                        }
